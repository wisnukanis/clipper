...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'login') {
    ...
    header('Location: /tiktok-demo.php');
    exit;
  }
  ...
  if (!empty($_GET['tiktok_code'])) {
    ...
    header('Location: /tiktok-demo.php?connected=1');
    exit;
  }
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'publish') {
    ...
    header('Location: /tiktok-demo.php?published=1');
    exit;
  }
...
if (isset($_GET['published'])) $message = 'Video demo berhasil dikirim ke TikTok Content Posting API Sandbox.';
?>
<!doctype html>
<html lang="id">
<head>
  ...
  <title>Clipper Emsa Pro TikTok Demo</title>
  <style>
    :root {
      color-scheme: light;
      --ink: #101828;
    ...
    * { box-sizing: border-box; }
    ...
    a { color: inherit; }
    header { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 18px 28px; background: #fff; border-bottom: 1px solid var(--line); }
    .brand { display: flex; align-items: center; gap: 10px; font-weight: 800; text-decoration: none; }
    .mark { display: grid; place-items: center; width: 38px; height: 38px; border-radius: 8px; background: var(--ink); color: #fff; }
    ...
    .panel { background: var(--panel); border: 1px solid var(--line); border-radius: 10px; padding: 22px; box-shadow: 0 16px 40px rgba(16, 24, 40, .06); }
    ...
    .btn { appearance: none; border: 1px solid var(--line); background: #fff; color: var(--ink); border-radius: 8px; padding: 11px 15px; font-weight: 800; text-decoration: none; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; min-height: 42px; }
    .btn.primary { background: var(--ink); color: #fff; border-color: var(--ink); }
    .btn:disabled { opacity: .55; cursor: not-allowed; }
    .row { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
    .status { display: inline-flex; align-items: center; border: 1px solid var(--line); border-radius: 999px; padding: 5px 10px; font-size: 13px; font-weight: 800; color: var(--muted); }
    ...
    .step { border: 1px solid var(--line); border-radius: 8px; padding: 14px; background: #fff; min-height: 112px; }
    .step span { display: grid; place-items: center; width: 26px; height: 26px; border-radius: 999px; background: #eef4ff; color: #004eeb; font-weight: 800; margin-bottom: 10px; }
    ...
    footer { border-top: 1px solid var(--line); padding: 18px 28px; color: var(--muted); display: flex; gap: 16px; flex-wrap: wrap; }
...
<body>
  <header>
    ...
    <nav>
      ...
      <a href="/tiktok-demo.php">TikTok Demo</a>
    </nav>
  </header>
  ...
  <main>
    ...
    <section class="hero">
      <div class="panel">
        <p class="eyebrow">TikTok Sandbox Review</p>
        <h1>Clipper Emsa Pro</h1>
        <p class="lead">Dashboard demo untuk memperlihatkan alur website resmi, login aplikasi, Connect TikTok, authorize scope, callback, pilih video, publish, dan hasil sandbox.</p>
        <div class="row">
    ...
    <section class="steps">
      ...
      <div class="step"><span>2</span><strong>Login demo</strong><small>User masuk ke dashboard aplikasi.</small></div>
    ...
    <section class="grid">
      <div class="panel">
        <h2>1. Login Dashboard</h2>
        <?php if ($loggedIn): ?>
          <p class="muted">Session demo aktif.</p>
          <form method="post"><input type="hidden" name="action" value="logout"><button class="btn" type="submit">Logout Demo</button></form>
        <?php else: ?>
          <p class="muted">Klik tombol ini saat rekaman demo untuk memperlihatkan login ke dashboard aplikasi.</p>
          <form method="post"><input type="hidden" name="action" value="login"><button class="btn primary" type="submit">Login Demo</button></form>
        <?php endif; ?>
      </div>
      ...
      <div class="panel">
        ...
        <p class="muted">Mode demo memakai inbox upload Sandbox agar user menyelesaikan posting dari aplikasi TikTok.</p>
        <form method="post">
