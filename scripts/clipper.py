...
try:
    from src.thumbnail_renderer import generate_thumbnails_for_clip
    THUMBNAIL_RENDERER_ENABLED = True
except ImportError:
    THUMBNAIL_RENDERER_ENABLED = False
...
def cfg():
    return {
        ...
        "background_music_enabled": parse_int(os.environ.get("BACKGROUND_MUSIC_ENABLED"), 1),
        "background_music_volume": parse_float(os.environ.get("BACKGROUND_MUSIC_VOLUME"), 0.10),
        "original_audio_volume": parse_float(os.environ.get("ORIGINAL_AUDIO_VOLUME"), 1.0),
        ...
        "viral_search_enabled": parse_int(os.environ.get("VIRAL_SEARCH_ENABLED"), 1),
        "viral_search_theme": os.environ.get("VIRAL_SEARCH_THEME", "").strip(),
        "viral_search_count": parse_int(os.environ.get("VIRAL_SEARCH_COUNT"), 12),
        "viral_min_views": parse_int(os.environ.get("VIRAL_MIN_VIEWS"), 10000),
        "cuopt_enabled": parse_int(os.environ.get("CUOPT_ENABLED"), 1),
        "cuopt_time_limit": parse_int(os.environ.get("CUOPT_TIME_LIMIT_SECONDS"), 20),
    }
...
def find_important_clips(segments, config, source_url=None, job_id=None):
    if not config["gemini_keys"]:
        raise RuntimeError("GEMINI_API_KEYS belum diisi di .env")

    viral_references = find_viral_references(segments, config, source_url)
    candidate_count = max(config["clip_count"] * 4, config["clip_count"] + 6)
    viral_context = format_viral_context(viral_references)
    prompt = f"""
Anda adalah editor video short-form profesional.

Tugas:
Pilih {candidate_count} kandidat bagian terbaik dari transkrip video untuk dijadikan clip pendek.
Nanti sistem optimizer akan memilih {config['clip_count']} final clip dari kandidat ini.
...
- Prioritaskan pola dari referensi viral: judul pendek, rasa penasaran, konflik, perubahan emosi, atau pelajaran jelas.
- Kandidat harus saling bervariasi; jangan semua dari momen yang terlalu berdekatan.
- Jangan pilih konten murottal, mengaji, tilawah, tadarus, bacaan ayat Al-Quran, qari/qariah, tahsin, atau tajwid. Jika transkrip didominasi bacaan suci, kembalikan array kosong [].
...
Referensi video viral sesuai tema:
{viral_context}
...
"""
    ...
    for index, key in enumerate(config["gemini_keys"]):
        try:
            ...
            candidates = validate_clips(clips, segments, config, limit=False)
            selected = optimize_clip_selection(candidates, segments, config, viral_references)
            if job_id:
                write_viral_report(job_id, viral_references, candidates, selected)
            return selected
        except Exception as exc:
            last_error = exc
...
def find_viral_references(segments, config, source_url=None):
    if int(config.get("viral_search_enabled", 1)) != 1:
        return []

    theme = config.get("viral_search_theme") or infer_viral_theme(segments)
    if not theme:
        return []

    query = f"{theme} shorts viral indonesia"
    limit = max(3, min(30, int(config.get("viral_search_count", 12))))
    log_step(f"Mencari referensi video viral: {query}")

    try:
        result = run_ytdlp(["-J", "--skip-download", "--flat-playlist", f"ytsearch{limit}:{query}"], capture=True)
        data = json.loads(result.stdout or "{}")
    except Exception as exc:
        log_warn(f"Pencarian viral gagal, lanjut tanpa referensi: {exc}")
        return []

    source_id = extract_video_id(source_url)
    references = []
    for entry in data.get("entries") or []:
        title = clean_public_clip_text(entry.get("title"), "")
        if not title:
            continue
        if source_id and source_id == str(entry.get("id") or ""):
            continue

        views = parse_int(entry.get("view_count"), 0)
        if views < int(config.get("viral_min_views", 10000)):
            continue

        duration = parse_float(entry.get("duration"), 0.0)
        score = viral_reference_score(entry)
        references.append(
            {
                "title": title[:120],
    ...
    top = references[:limit]
    log_info(f"Referensi viral ditemukan: {len(top)} video")
    return top


def infer_viral_theme(segments):
    text = " ".join(str(segment.get("text") or "") for segment in segments[:120]).casefold()
    themes = [
        ("podcast lucu hikmah", ("lucu", "ketawa", "ngakak", "komedi", "bercanda")),
        ("motivasi hidup", ("gagal", "hidup", "sukses", "mental", "tekanan", "semangat")),
        ("kisah inspiratif", ("cerita", "perjalanan", "masa lalu", "berubah", "pelajaran")),
        ("renungan hidup", ("renungan", "makna", "sabar", "ikhlas", "takdir")),
        ("fakta menarik", ("fakta", "data", "riset", "ternyata", "sains")),
    ]
    for theme, keywords in themes:
        if any(keyword in text for keyword in keywords):
            return theme
    return "podcast inspiratif"
...
def viral_reference_score(entry):
    views = max(0, parse_int(entry.get("view_count"), 0))
    duration = max(1.0, parse_float(entry.get("duration"), 0.0))
    short_bonus = 1.15 if duration <= 90 else 0.85 if duration <= 240 else 0.55
    title = str(entry.get("title") or "").casefold()
    hook_bonus = 1.0 + min(0.45, sum(1 for word in ("ternyata", "kenapa", "rahasia", "gagal", "hidup", "viral", "jangan") if word in title) * 0.09)
    return (views ** 0.5) * short_bonus * hook_bonus


def format_viral_context(references):
    if not references:
        return "- Tidak ada referensi eksternal; gunakan pola short-form umum."
    lines = []
    for index, item in enumerate(references[:10], start=1):
        lines.append(f"- {index}. {item['title']} | views={item['views']} | duration={round(item['duration'], 1)}s")
    return "\n".join(lines)


def write_viral_report(job_id, references, candidates, selected):
    report = {
        ...
        "selected": [
            {
                ...
                "end": clip.get("end"),
                "viralScore": clip.get("viralScore"),
                "optimizationScore": clip.get("optimizationScore"),
            }
            for clip in selected
        ],
    }
    path = ROOT / "temp" / f"{job_id}-viral-optimization.json"
    path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")


def validate_clips(clips, segments, config, limit=True):
    ...
    for index, clip in enumerate(clips):
        ...
        duration = end - start
        if duration < float(config["min_clip_seconds"]) * 0.75:
            continue
        if duration > float(config["max_clip_seconds"]) * 1.35:
            end = start + float(config["max_clip_seconds"])

        title = clean_clip_title(clip.get("title"), segments, start, end, index)
        hook = clean_public_clip_text(clip.get("hook"), title)
        caption = clean_public_clip_text(clip.get("caption"), "")
        reason = clean_public_clip_text(clip.get("reason"), "")
        ...
        result.append(
            {
                ...
                "hook": hook,
                "caption": caption,
                "viralScore": parse_float(clip.get("viralScore"), 0.0),
            }
        )

    return result[: config["clip_count"]] if limit else result


def optimize_clip_selection(candidates, segments, config, viral_references=None):
    if not candidates:
        return []

    scored = []
    for index, clip in enumerate(candidates):
        score = score_clip_candidate(clip, segments, config, viral_references or [])
        scored.append({**clip, "optimizationScore": round(score, 4), "_candidateIndex": index})

    cuopt_requested = int(config.get("cuopt_enabled", 1)) == 1 and len(scored) > config["clip_count"]
    if cuopt_requested:
        selected = optimize_with_cuopt(scored, config)
        if selected:
            return strip_internal_clip_fields(selected)

    if cuopt_requested:
        log_warn("cuOpt tidak tersedia/gagal; pakai fallback optimizer lokal.")
    else:
        log_info("Pakai optimizer lokal.")
    return strip_internal_clip_fields(select_non_overlapping_by_score(scored, config["clip_count"]))


def score_clip_candidate(clip, segments, config, viral_references):
    duration = max(1.0, float(clip["end"]) - float(clip["start"]))
    target_duration = (float(config["min_clip_seconds"]) + float(config["max_clip_seconds"])) / 2.0
    duration_score = max(0.0, 1.0 - abs(duration - target_duration) / max(target_duration, 1.0))

    text = " ".join(
        str(clip.get(key) or "")
        for key in ("title", "hook", "caption", "reason")
    ).casefold()
    segment_text = clip_text_from_segments(segments, clip["start"], clip["end"]).casefold()
    haystack = f"{text} {segment_text}"

    hook_words = ("ternyata", "kenapa", "jangan", "rahasia", "gagal", "hidup", "tekanan", "takut", "salah", "berubah")
    emotion_words = ("sedih", "takut", "marah", "lucu", "kaget", "bangga", "sakit", "tertekan", "menyesal")
    insight_words = ("belajar", "pelajaran", "makna", "cara", "alasan", "fakta", "mindset", "sadar")
    keyword_score = min(1.0, count_keywords(haystack, hook_words) * 0.12 + count_keywords(haystack, emotion_words) * 0.10 + count_keywords(haystack, insight_words) * 0.08)
    hook_length_score = 1.0 if 4 <= len(str(clip.get("hook") or "").split()) <= 18 else 0.55
    reference_score = viral_similarity_score(haystack, viral_references)
    ai_score = clamp(parse_float(clip.get("viralScore"), 0.0), 0.0, 1.0)

    return (
        0.24 * duration_score
        + 0.26 * keyword_score
        + 0.18 * hook_length_score
        + 0.22 * reference_score
        + 0.10 * ai_score
    )


def clip_text_from_segments(segments, start, end):
    texts = []
    for segment in segments:
...
def viral_similarity_score(text, references):
    if not references:
        return 0.5
    ...
    for reference in references[:10]:
        ref_tokens = tokenize_for_similarity(reference.get("title", ""))
        if not ref_tokens:
            continue
        overlap = len(tokens & ref_tokens) / max(1, len(tokens | ref_tokens))
        best = max(best, overlap)
    return min(1.0, best * 3.0)


def tokenize_for_similarity(value):
    stopwords = {"yang", "dan", "di", "ke", "dari", "ini", "itu", "untuk", "dengan", "aku", "kamu", "nya", "the", "a"}
...
def optimize_with_cuopt(candidates, config):
    try:
        from cuopt.linear_programming.problem import INTEGER, MAXIMIZE, LinearExpression, Problem
        from cuopt.linear_programming.solver_settings import SolverSettings
    except Exception as exc:
        log_warn(f"Import cuOpt gagal: {exc}")
        return []

    try:
        problem = Problem("ViralClipSelection")
        variables = [
            problem.addVariable(lb=0, ub=1, vtype=INTEGER, name=f"clip_{index}")
            for index, _ in enumerate(candidates)
        ]
        problem.addConstraint(LinearExpression(variables, [1.0] * len(variables), constant=0.0) == int(config["clip_count"]), name="clip_count")

        min_duration = float(config["min_clip_seconds"])
        max_duration = float(config["max_clip_seconds"])
        for index, clip in enumerate(candidates):
            duration = float(clip["end"]) - float(clip["start"])
            if duration < min_duration or duration > max_duration:
                problem.addConstraint(variables[index] == 0, name=f"duration_{index}")

        for left in range(len(candidates)):
            for right in range(left + 1, len(candidates)):
                if clips_overlap(candidates[left], candidates[right], gap=2.0):
                    problem.addConstraint(variables[left] + variables[right] <= 1, name=f"overlap_{left}_{right}")

        coeffs = [float(clip.get("optimizationScore", 0.0)) for clip in candidates]
        problem.setObjective(LinearExpression(variables, coeffs, constant=0.0), sense=MAXIMIZE)

        settings = SolverSettings()
        settings.set_parameter("time_limit", int(config.get("cuopt_time_limit", 20)))
        settings.set_parameter("mip_relative_gap", 0.01)
        problem.solve(settings)

        if problem.Status.name not in ["Optimal", "FeasibleFound"]:
            log_warn(f"cuOpt status: {problem.Status.name}")
            return []

        chosen = [
            clip
            for variable, clip in zip(variables, candidates)
            if float(variable.getValue() or 0.0) > 0.5
        ]
        chosen.sort(key=lambda item: item.get("optimizationScore", 0.0), reverse=True)
        log_info(f"cuOpt memilih {len(chosen)} clip final. Objective={problem.ObjValue}")
        return chosen
    except Exception as exc:
        log_warn(f"Optimasi cuOpt gagal: {exc}")
        return []


def clips_overlap(left, right, gap=0.0):
    return float(left["end"]) + gap > float(right["start"]) and float(right["end"]) + gap > float(left["start"])


def select_non_overlapping_by_score(candidates, clip_count):
    selected = []
    for clip in sorted(candidates, key=lambda item: item.get("optimizationScore", 0.0), reverse=True):
        if any(clips_overlap(clip, other, gap=2.0) for other in selected):
            continue
...
def strip_internal_clip_fields(clips):
    result = []
    for clip in clips:
        clean = {key: value for key, value in clip.items() if not key.startswith("_")}
        result.append(clean)
    return result


def clean_clip_title(title, segments, start, end, index):
    value = re.sub(r"\s+", " ", str(title or "")).strip()

    if not value or has_bad_public_text_marker(value):
        value = title_from_segments(segments, start, end)

    value = re.sub(r"^(fallback\s+lokal\s*)+", "", value, flags=re.IGNORECASE).strip(" -:.,")
    value = re.sub(r"\s+", " ", value).strip()

    return value[:80].strip(" -:.,") or f"Clip {index + 1}"


def clean_public_clip_text(value, fallback=""):
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    if not text or has_bad_public_text_marker(text):
        text = fallback
    text = re.sub(r"^(fallback\s+lokal\s*)+", "", str(text or ""), flags=re.IGNORECASE).strip(" -:.,")
    return re.sub(r"\s+", " ", text).strip()


def has_bad_public_text_marker(value):
...
def title_from_segments(segments, start, end):
    texts = []
    for segment in segments:
...
Style: Caption,Segoe UI Semibold,{font_size},&H0000FFFF,&H0000FFFF,&H00111111,&H66000000,-1,0,0,0,100,100,0,0,1,4,1,2,80,80,{margin_v},1
...
def render_clip(source_clip, ass_path, clip, index, config, job_id):
    ...
    try:
        run_render(source_clip, final_clip, subtitle_vf, config, clip)
    except subprocess.CalledProcessError as exc:
        log_warn(f"Render smart/subtitle gagal, coba crop tengah dengan subtitle: {exc}")
        try:
            run_render(source_clip, final_clip, f"{centered_vf},subtitles='{subtitle_path}'", config, clip)
        except subprocess.CalledProcessError as fallback_exc:
            log_warn(f"Render subtitle gagal, fallback tanpa hardsub: {fallback_exc}")
            run_render(source_clip, final_clip, centered_vf, config, clip)

    return source_clip, final_clip, crop_path


def generate_clip_thumbnail(clip, output_dir, frame_path=None):
    """Generate branded thumbnail untuk clip menggunakan template sistem 3 warna."""
    if not THUMBNAIL_RENDERER_ENABLED:
        return {}
    try:
        result = generate_thumbnails_for_clip(
            clip=clip,
            output_dir=output_dir,
            frame_path=frame_path,
        )
        if result.get("thumb_shorts"):
            log_info(f"Thumbnail generated: {result['thumb_shorts']}")
        return result
    except Exception as exc:
        log_warn(f"Thumbnail generation failed: {exc}")
        return {}


def run_render(source_clip, final_clip, vf, config, clip=None):
    music_path = choose_background_music(clip or {}, config)
    command = [
        "ffmpeg",
        "-y",
        "-i",
    ...
    if music_path:
        original_volume = clamp(config.get("original_audio_volume", 1.0), 0.0, 2.0)
        music_volume = clamp(config.get("background_music_volume", 0.10), 0.0, 1.0)
        command.extend(
            [
                "-filter_complex",
                (
        ...
        log_info(f"Background music: {music_path.relative_to(ROOT)}")
...
def choose_background_music(clip, config):
    if int(config.get("background_music_enabled", 1)) != 1:
        return None

    mood = infer_music_mood(clip)
    music_dir = ROOT / "assets" / "music" / mood
    if not music_dir.exists():
        return None

    extensions = {".mp3", ".wav", ".m4a", ".aac", ".ogg", ".flac"}
    tracks = sorted(path for path in music_dir.iterdir() if path.is_file() and path.suffix.lower() in extensions)
    return tracks[0] if tracks else None


def infer_music_mood(clip):
    category = str(clip.get("category") or clip.get("content_type") or "").casefold()
    text = " ".join(
        str(clip.get(key) or "")
        for key in ("title", "hook", "caption", "reason", "content_tone")
    ).casefold()
    haystack = f"{category} {text}"

    if any(word in haystack for word in ("podcast", "lucu", "ngakak", "ketawa", "kocak", "blunder")):
        return "podcast_lucu_hikmah"
    if any(word in haystack for word in ("renungan", "reflektif", "adem", "makna")):
        return "renungan"
    if any(word in haystack for word in ("hikmah", "inspiratif", "ceramah", "nasihat", "sabar", "ikhlas")):
        return "inspiratif_hikmah"
...
def main():
    ...
    if args.range:
        ...
        if cached_clips:
            ...
            clips = find_important_clips(segments, config, args.url, job_id)
    ...
    for index, clip in enumerate(clips):
        ...
        thumbnail_output_dir = str(final_clip.parent)
        clip_thumbnails = generate_clip_thumbnail(
            clip=clip,
            output_dir=thumbnail_output_dir,
            frame_path=None,
        )
        if clip_thumbnails:
            clip["thumbnailShorts"] = clip_thumbnails.get("thumb_shorts", "")
            clip["thumbnailYoutube"] = clip_thumbnails.get("thumb_youtube", "")
        outputs.append(
            {
                "index": index + 1,
                "title": clip["title"],
                ...
                "viralScore": clip.get("viralScore", 0.0),
                "optimizationScore": clip.get("optimizationScore", 0.0),
                ...
                "thumbnailShorts": clip.get("thumbnailShorts", ""),
                "thumbnailYoutube": clip.get("thumbnailYoutube", ""),
            }
        )
